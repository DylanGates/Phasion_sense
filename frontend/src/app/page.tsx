...
const FALLBACK_IMGS = [
  "https://images.unsplash.com/photo-1539109132314-347752d87b40?q=80&w=800&auto=format&fit=crop",
...
function imgUrl(path: string | undefined, fallback: string): string {
...
export default async function Page() {
  ...
  try {
    const [itemsRes, campaignsRes] = await Promise.all([
      fetch(`${API_BASE}/merchants/${MERCHANT}/items`, { next: { revalidate: 60 } }),
      fetch(`${API_BASE}/campaigns?merchant_id=${MERCHANT}`, { next: { revalidate: 60 } }),
    ])
  ...
  const heroImg = imgUrl(items[0]?.image_urls?.[0], FALLBACK_HERO)

  const categoryItems = [
    { title: "New Arrivals",    img: imgUrl(items[0]?.image_urls?.[0], FALLBACK_IMGS[0]) },
    { title: "The Casual Edit", img: imgUrl(items[4]?.image_urls?.[0], FALLBACK_IMGS[1]) },
    { title: "Best-Sellers",    img: imgUrl(items[8]?.image_urls?.[0], FALLBACK_IMGS[2]) },
  ]

  const productGrid = items.slice(0, 5).map((p: any) => ({
    id: p.id,
    name: p.name,
    price: `GH₵${(p.price_minor / 100).toFixed(2)}`,
    image: imgUrl(p.image_urls?.[0], FALLBACK_IMGS[0]),
  }))

  const lifestyleLeft  = imgUrl(items[11]?.image_urls?.[0], FALLBACK_IMGS[3])
  const lifestyleRight = imgUrl(items[12]?.image_urls?.[0], FALLBACK_IMGS[4])

  const igImages = items.slice(5, 10).map((p: any, i: number) =>
    imgUrl(p?.image_urls?.[0], FALLBACK_IMGS[i % FALLBACK_IMGS.length])
  )
  while (igImages.length < 5) igImages.push(FALLBACK_IMGS[igImages.length % FALLBACK_IMGS.length])

  return (
    <AppShell>
      {/* Hero */}
      <section className="relative h-[92vh] w-full overflow-hidden">
        <Image src={heroImg} alt="Hero" fill priority className="object-cover object-top" unoptimized />
      ...
      <section className="py-32">
        <div className="container mx-auto px-6">
          ...
          {productGrid.length > 0 ? (
            ...
            <p className="text-muted-foreground text-sm text-center py-20">No items available at the moment.</p>
          )}
        </div>
      </section>
      ...
      <section className="container mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-10 py-12">
        <Link href="/catalog" className="group relative aspect-[3/4] overflow-hidden block">
          <Image src={lifestyleLeft} alt="The Smart Chic" fill unoptimized className="object-cover transition-transform duration-1000 group-hover:scale-105" />
        ...
        <Link href="/catalog" className="group relative aspect-[3/4] overflow-hidden block">
          <Image src={lifestyleRight} alt="Ready To Go" fill unoptimized className="object-cover transition-transform duration-1000 group-hover:scale-105" />
          <div className="absolute bottom-10 left-10 text-white font-serif">
